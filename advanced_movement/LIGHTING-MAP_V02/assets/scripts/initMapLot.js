async function initMapLot() {
    ...
    lottiUniciColor=lottiUnici(response);
    ...
    for (const marker of filteredMarkers) {
        ...
        coloreLotti = coloreLottoCorrente(lottiUniciColor);
        lottoColor = coloreMarkerCorrente(coloreLotti, globalCounter, response);
        let pinColor;
        if (marker.segnalazioni_in_corso.length > 0) {
            
            if(lottoColor.icona === ''){
                lottoColor.coloreGliph = '#ffff00'; 
                pinColor = new PinElement({
                    background: lottoColor.coloreBackground,
                    borderColor: '#000000',
                    scale: 1,
                    glyphColor: lottoColor.coloreGliph, 
                    //glyph: lottoColor.icona
                });
            }else{
                ...
                lottoColor.icona.insertBefore(newDiv, lottoColor.icona.firstChild);
                pinColor = new PinElement({
                    background: lottoColor.coloreBackground,
                    borderColor: '#000000',
                    scale: 1,
